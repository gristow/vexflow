...
Vex.Flow.Beam = (function() {
  ...
  Beam.prototype = {
    ...
    //        console.log("Forced partial on: " + note);
            return {
...
/*
          // If it can't be a partial (i.e. it's an 8th note) return falses:
          if(duration === "8") {
            console.log("Eighth!");
            return { left: false, right: false };
          }
*/
          // If no previous note, right partial & return:
          if(!prev_note) { right_partial = true; left_partial = false; }
          // If no next note, left partial & return:
          else if(!next_note) { left_partial = true; right_partial = false; }
          // Otherwise: partial in direction of note w/ most beams:
          else {
      ...
      for (var i = 0; i < this.notes.length; ++i) {
        ...
        var text = partial.left && partial.right ? "both" : "none";
        if(partial.left && !partial.right) text = "left";
        if(partial.right && !partial.left) text = "right";

//        console.log("index: " + i + ", partial: " + text);
